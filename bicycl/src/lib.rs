...
impl<'a> From<&'a bls12_381::Scalar> for Mpz {
    fn from(value: &'a bls12_381::Scalar) -> Self {
        let mut tmp = value.to_bytes();
        tmp.reverse();
        Mpz::from_bytes(&tmp)
    }
}
...
impl PartialOrd for Mpz {
    fn partial_cmp(&self, other: &Self) -> Option<std::cmp::Ordering> {
        match BICYCL::Mpz::cmpabs(&self.mpz, &*other.mpz) {
            autocxx::c_int(1..=i32::MAX) => Some(Ordering::Greater),
            autocxx::c_int(i32::MIN..=-1) => Some(Ordering::Less),
            autocxx::c_int(0) => Some(Ordering::Equal),
        }
    }
}
