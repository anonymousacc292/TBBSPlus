use super::{utils::compute_masked_arguments_to_multiply_n_party, ParticipantId};
...
impl<F: PrimeField, const SALT_SIZE: usize> Phase1<F, SALT_SIZE> {
    ...
    pub fn compute_randomness_and_arguments_for_multiplication<D: Default + DynDigest + Clone>(
        ...
        let mut masked_signing_key_share;
        let mut masked_r;
        if *flag {
            (masked_signing_key_share, masked_r) = compute_masked_arguments_to_multiply(
                signing_key,
                self.r,
                zero_shares,
                self.id,
            ...
            (masked_signing_key_share, masked_r) = compute_masked_arguments_to_multiply_n_party(
                signing_key,
                self.r,
                zero_shares,
                self.id,
