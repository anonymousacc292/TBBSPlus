...
impl<F: PrimeField, const SALT_SIZE: usize> Phase1<F, SALT_SIZE> {
    ...
    pub fn finish_for_bbs_plus<D: Default + DynDigest + Clone>(
        ...
        let (others, mut randomness, masked_signing_key_shares, masked_rs) =
            self.compute_randomness_and_arguments_for_multiplication::<D>(signing_key, flag)?;
...
pub mod tests {
    ...
    pub fn trusted_party_keygen_n_party<R: RngCore, F: PrimeField>(
        rng: &mut R,
        total: ParticipantId,
    ) -> (F, Vec<F>) {
    ...
    fn signing_t_out_of_n() {
        ...
        fn check(
            ...
            println!("One time setup {:?}", start.elapsed());
            ...
            for (i, round1) in round1s.into_iter().enumerate() {
                ...
                let start = Instant::now();
                let out = round1
                    .finish_for_bbs_plus::<Blake2b512>(&sk_shares[i], &true)
                    .unwrap();
        ...
        // check(&mut rng, ote_params, 2, 2, 1, 10, &gadget_vector);
        // check(&mut rng, ote_params, 5, 5, 1, 10, &gadget_vector);
        check(&mut rng, ote_params, 10, 11, 1, 10, &gadget_vector);
        // check(&mut rng, ote_params, 15, 15, 1, 10, &gadget_vector);
        // check(&mut rng, ote_params, 20, 20, 1, 10, &gadget_vector);
        // check(&mut rng, ote_params, 25, 25, 1, 10, &gadget_vector);
        // check(&mut rng, ote_params, 30, 30, 1, 10, &gadget_vector);
        // check(&mut rng, ote_params, 35, 35, 1, 10, &gadget_vector);
        // check(&mut rng, ote_params, 40, 40, 1, 10, &gadget_vector);
    }

    #[test]
    fn signing_n_out_of_n() {
        let mut rng = StdRng::seed_from_u64(0u64);
        const BASE_OT_KEY_SIZE: u16 = 128;
        const KAPPA: u16 = 256;
        const STATISTICAL_SECURITY_PARAMETER: u16 = 80;
        ...
        fn check(
            ...
            let start = Instant::now();
            let (sk, sk_shares) =
                trusted_party_keygen_n_party::<_, Fr>(rng, total_signers);
            println!("One time setup {:?}", start.elapsed());
            ...
            println!(
                "For a batch size of {} BBS+ signatures on messages of size {} and {} signers",
                sig_batch_size, message_count, total_signers
            );
            ...
            for i in 1..=total_signers {
                let start = Instant::now();
                let mut others = total_party_set.clone();
                others.remove(&i);
                let (round1, comm, comm_zero) = Phase1::<Fr, 256>::init_for_bbs_plus(
                    rng,
                    sig_batch_size,
            ...
            for (i, round1) in round1s.into_iter().enumerate() {
                ...
                let start = Instant::now();
                let out = round1
                    .finish_for_bbs_plus::<Blake2b512>(&sk_shares[i], &false)
                    .unwrap();
                phase1_times.insert(id, *phase1_times.get(&id).unwrap() + start.elapsed());
                expected_sk += out.masked_signing_key_shares.iter().sum::<Fr>();
                round1outs.push(out);
            ...
            for k in 0..sig_batch_size as usize {
                ...
                let mut shares = vec![];
                let start = Instant::now();
                for i in 0..total_signers as usize {
                    let id = round1outs[i].id;
                    let start = Instant::now();
                    let share = BBSPlusSignatureShare::new(
                        &messages,
        ...
        // check(&mut rng, ote_params,  2, 1, 10, &gadget_vector);
        // check(&mut rng, ote_params,  5, 1, 10, &gadget_vector);
        check(&mut rng, ote_params,  10, 1, 10, &gadget_vector);
        // check(&mut rng, ote_params, 15, 15, 1, 10, &gadget_vector);
        // check(&mut rng, ote_params, 20, 20, 1, 10, &gadget_vector);
        // check(&mut rng, ote_params, 25, 25, 1, 10, &gadget_vector);
        // check(&mut rng, ote_params, 30, 30, 1, 10, &gadget_vector);
